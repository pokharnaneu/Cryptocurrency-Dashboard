      ...
      axios.get(`/api/${coin_id}`)
        ...
        //  for (var i = 0; i < 1; i++){
         console.log("posts info box",posts);
            ...
            const change = price - data[0].y;  //current price minus price before 1month(app.js--data)
            const changeP = (price - data[0].y) / data[0].y * 100; 
            console.log("price",price);           
            console.log("change",change);
            console.log("changep",changeP);
            this.setState({
              currentPrice: (posts.price - 0).toLocaleString('us-EN',{ style: 'currency', currency: 'USD' }),
